...
| Item | Detail | Status |
|------|--------|--------|
| Commit this file | ROADMAP.md was authored 2026-06-12 but never committed; it must live in the repo, not in chat history | Approved |
...
|---|------|--------|----------|
| 1 | **Hybrid search** (tsvector + RRF) | Add Postgres full-text search over chunks; fuse keyword + vector rankings via reciprocal rank fusion. Acceptance: `grounded-02` flips green, no regressions on the full eval set. | grounded-02 failed 3 consecutive runs — "schedule" query never retrieved the section literally titled "Scan Scheduling" |
| 2 | **Embeddable widget** (Phase 6) | Public rate-limited chat endpoint + iframe widget per existing CLAUDE.md spec. This is what makes a first paying customer mechanically possible. | Planned since Feb; README promises it |

## 3. Proposed (awaiting Aditya's approval)

| Item | Detail | Why |
|------|--------|-----|
| Overview-detector coverage patterns | Add coverage phrasings ("not covered", "does it cover", "what's missing", "doesn't include") to `is_overview_question` in rag.py so coverage questions always receive document structure. | Verified 2026-07-03: these phrasings are absent from the detector; the 2026-06-11 coverage answer made correct absence claims without structure — right by luck. Absence claims are only groundable via structure. |
...
- **Coverage-question grounding** — absence claims not guaranteed to be structure-grounded; remediated by the Proposed overview-detector patterns + coverage-01 eval case.
